...
typedef struct
    {
    byte        Cluster;        // Mux chip address
    byte        Slice;          // Mux output select
    byte        Channel;        // I2C address of device
    byte        NumberDtoA;     // Device count on channel: 1 = MCP4725, 4 = MCP4728
    byte        NumberAtoD;
    byte        NumberDigital;  // number of I/O channels
    const char* Name;
    } I2C_LOCATION_T;
...
private:
    ...
    void BusMux         (byte cluster, byte channel);
    void EndBusMux      (byte cluster);
